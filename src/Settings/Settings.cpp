...
const char DEFAULT_WALLET_FILE_NAME[] = "stardustcoinwallet.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "pool.stardustcoin.org:3333";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("stardustcoinwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("stardustcoinwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("stardustcoinwallet.desktop");
  ...
  res = autorunSettings.contains("StardustCoinWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("StardustCoinWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.stardustcoin.stardustcoin scientistwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=StardustCoin Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("StardustCoinWallet", appPath);
    } else {
      autorunSettings.remove("StardustCoinWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\stardustcoin", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:stardustcoin");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\stardustcoin\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\stardustcoin\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("stardustcoinwallet.cfg"));
